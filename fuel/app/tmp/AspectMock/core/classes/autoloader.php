...
class Autoloader
{
	...
	{ if (($__am_res = __amock_before(get_called_class(), __CLASS__, __FUNCTION__, array($class), true)) !== __AM_CONTINUE__) return $__am_res; 
		...
		elseif ($full_class = static::find_core_class($class))
		{
			...
			{
				include \Go\Instrument\Transformer\FilterInjectorTransformer::rewrite( static::prep_path(static::$classes[strtolower($full_class)]), '/Users/kakehi/zisyu/fuelphpStudy/fuel/core/classes');
			}
	...
	{ if (($__am_res = __amock_before(get_called_class(), __CLASS__, __FUNCTION__, array($class, $file), true)) !== __AM_CONTINUE__) return $__am_res; 
		...
		if ($file)
		{
			include \Go\Instrument\Transformer\FilterInjectorTransformer::rewrite( $file, '/Users/kakehi/zisyu/fuelphpStudy/fuel/core/classes');
		}
