...
class View
{
	...
	{ if (($__am_res = __amock_before(get_called_class(), __CLASS__, __FUNCTION__, array($file, $data, $auto_filter), true)) !== __AM_CONTINUE__) return $__am_res; 
	...
	{ if (($__am_res = __amock_before($this, __CLASS__, __FUNCTION__, array($file, $data, $filter), false)) !== __AM_CONTINUE__) return $__am_res; 
	...
	{ if (($__am_res = __amock_before($this, __CLASS__, __FUNCTION__, array($file_override), false)) !== __AM_CONTINUE__) return $__am_res; 
		$clean_room = function($__file_name, array $__data)
		{
			...
			try
			{
				...
				include \Go\Instrument\Transformer\FilterInjectorTransformer::rewrite( $__file_name, '/Users/kakehi/zisyu/fuelphpStudy/fuel/core/classes');
			}
			catch (\Exception $e)
			{
	...
	{ if (($__am_res = __amock_before($this, __CLASS__, __FUNCTION__, array($scope), false)) !== __AM_CONTINUE__) return $__am_res; 
	...
	{ if (($__am_res = __amock_before($this, __CLASS__, __FUNCTION__, array($var), false)) !== __AM_CONTINUE__) return $__am_res; 
	...
	{ if (($__am_res = __amock_before(get_called_class(), __CLASS__, __FUNCTION__, array($key, $value, $filter), true)) !== __AM_CONTINUE__) return $__am_res; 
		if (is_array($key))
		{
	...
	{ if (($__am_res = __amock_before($this, __CLASS__, __FUNCTION__, array($filter), false)) !== __AM_CONTINUE__) return $__am_res; 
		if (func_num_args() == 0)
		{
	...
	{ if (($__am_res = __amock_before($this, __CLASS__, __FUNCTION__, array($file, $reverse), false)) !== __AM_CONTINUE__) return $__am_res; 
		// reset the filename
		$this->file_name = null;
	...
	{ if (($__am_res = __amock_before($this, __CLASS__, __FUNCTION__, array($key, $value, $filter), false)) !== __AM_CONTINUE__) return $__am_res; 
		if (is_array($key))
		{
			foreach ($key as $name => $value)
			{
